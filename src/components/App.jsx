...
const App = () => {
  ...
  return (
    <div className='space-grotesk'>
      <button onClick={handleUp} className='text-[20px] bg-[#98BFFC] fixed bottom-10 md:m-7 md:my-3 m-6 my-7 p-3 rounded-md right-0 z-40'><FaArrowUp /></button>
      ...
      <div>
        {allData && allData.sort((a, b) => b.createdAt.seconds - a.createdAt.seconds).filter(item => (item.name.includes(target) && item.category.includes(category) && (item.info.price > price["min"] && item.info.price <= price["max"]))).length > 0 ?
          ...
          : <h3 className='text-[20px] text-center py-20'>Hal hazırda aktiv olan elan yoxdur</h3>
        }
      </div>
